...
- 수락 기준과 완료 조건 정의
- **API 연동 및 비즈니스 로직 구현 Task는 반드시 `## 테스트 체크리스트 (Playwright MCP)` 섹션 포함**
- **구현 완료 후 Playwright MCP 테스트 수행 필수 — 테스트 통과 없이 Task 완료 불가**
...
| Task 유형 | 테스트 필수 여부 | 테스트 방법 |
|-----------|----------------|------------|
| API 연동 (외부 서비스, DB) | **필수** | Playwright MCP E2E |
| 비즈니스 로직 (필터링, 계산, 변환) | **필수** | Playwright MCP + 화면 검증 |
| 인증·권한 시스템 | **필수** | Playwright MCP E2E |
| 사용자 플로우 (폼 제출, 네비게이션) | **필수** | Playwright MCP E2E |
| UI 컴포넌트 렌더링 | 권장 | 브라우저 스냅샷 확인 |
| 타입 정의, 환경 변수 설정 | 불필요 | - |

#### **테스트 체크리스트 작성 형식**

API 연동 및 비즈니스 로직 Task의 구현 사항 하단에 다음 형식으로 추가:
...
- [ ] [기능명]: [기대 동작 설명]
  - 시나리오: [테스트 단계 서술]
  - 기대 결과: [화면/응답에서 확인할 내용]

### 엣지 케이스
- [ ] 빈 데이터 상태에서 UI 정상 렌더링
- [ ] 잘못된 입력값 처리 (에러 메시지 노출 등)
- [ ] 네트워크 오류 또는 API 실패 시 fallback 동작

### 통합 검증
- [ ] 전체 사용자 플로우 처음부터 끝까지 수행
- [ ] 이전 Task 기능과 충돌 없음 확인
```

#### **테스트 수행 순서**

1. 개발 서버 실행 (`npm run dev`)
2. Playwright MCP로 브라우저 열기
3. 테스트 체크리스트 항목 순서대로 시나리오 수행
4. 각 항목 통과 시 `[ ]` → `[x]` 체크
5. **실패 항목 발견 시 구현 재검토 후 재테스트**
6. 전체 항목 통과 후 Task 완료 처리
...
- 작업 파일의 명세서를 따름
- 기능 구현
- **구현 후 반드시 테스트 수행** — 구현과 테스트는 항상 세트로 진행
  - API 연동 및 비즈니스 로직: Playwright MCP E2E 테스트 필수
  - UI 컴포넌트: 브라우저 렌더링 및 인터랙션 동작 확인
  - 필터링·검색·상태 관리: 사용자 시나리오 기반 동작 검증
- **테스트 실패 시 해당 구현 재검토 후 재테스트** — 통과 없이 다음 단계 진행 불가
...
5. **테스트**: API 연동·비즈니스 로직 Task는 반드시 `## 테스트 체크리스트 (Playwright MCP)` 섹션 포함
...
- 측정 가능한 완료 기준 제시
- **API 연동 또는 비즈니스 로직 포함 Task**: 구현 사항 하단에 `## 테스트 체크리스트 (Playwright MCP)` 섹션 반드시 추가
