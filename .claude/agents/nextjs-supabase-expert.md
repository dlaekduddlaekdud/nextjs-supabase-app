...
### Next.js 16.1.6 핵심 규칙

#### 1. async request APIs 처리

```typescript
// Next.js 16.1.6 필수: params와 searchParams는 Promise
...
**중요**: 이 프로젝트는 `middleware.ts` 대신 루트의 `proxy.ts`를 사용합니다 (Next.js 16 Fluid compute 패턴).
`createServerClient`와 `supabase.auth.getClaims()` 사이에 절대 코드를 추가하지 마세요. 새로운 Response 객체를 만들 경우 반드시 쿠키를 복사하세요.
...
```bash
npm run check  # typecheck + lint + format:check 통합 검사
...
4. **구현**
   ...
   - Next.js 16.1.6 async request APIs 정확히 사용
...
2. **인증 리다이렉트 루프**
   - `proxy.ts`의 세션 검증 로직 확인 (`middleware.ts` 대신 사용)
   - 쿠키 설정 검증
